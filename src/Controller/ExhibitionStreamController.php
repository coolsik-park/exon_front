...
class ExhibitionStreamController extends AppController
{
    ...
    public function tmpChapter($id = null, $exhibition_users_id = null, $cert = null)
    {
        ...
        $current = $ExhibitionUsers->get($exhibition_users_id);
        $exhibitionUsers = $ExhibitionUsers->find('all')->where(['users_email' => $current->users_email, 'exhibition_id IN' => [295, 296, 297]])->toArray();

        $this->set(compact('id', 'exhibition_users_id', 'cert', 'tmp_exhibitions', 'exhibition', 'tmp_exhibition_streams', 'exhibitionUsers'));
    }
    ...
    public function setExhibitionVod($exhibition_id = null)
    {
        ...
        $ExhibitionVod = $this->getTableLocator()->get('ExhibitionVod');
        $exhibitionVod = $ExhibitionVod->find('all')->where(['exhibition_id' => $exhibition_id, 'parent_id IS' => null])->order(['idx' => 'ASC']);
        $exhibitionVod->contain([
            'ChildExhibitionVod' => [
                'sort' => ['ChildExhibitionVod.idx' => 'ASC']
            ]
